#!/usr/bin/perl -w 
#
# First we explicitly switch off the Icinga embbeded Perl Interpreter
# icinga: -epn
# ######################### pnp4n_send_service_mail.pl ################ #
# Date    : May 29, 2012                                                #
# Purpose : Script to send out Icinga service e-mails.\n";              #
# Author  : Frank Migge (support at frank4dd dot com), Robert Becht     #
# Help    : http://nagios.frank4dd.com/howto                            #
# Licence : GPL - http://www.fsf.org/licenses/gpl.txt                   #
#           Written for and verified with Icinga version 3.3.1          #
...
# Version : 1.3 use environment variables for Icinga data handoff       #
...
my $mail_sender        = "Icinga Monitoring <icinga\@openology.net>";
# The Icinga CGI URL for integrated service links
my $icinga_cgiurl      = "https://service.icinga.openology.net/cgi-bin";
...
# ########################################################################
my $test_host          = "server-prod2";    # existing host in PNP4Nagios
my $test_service       = "check_cpu"; # existing services in PNP4nagios
...
# ########################################################################
my $pnp4nagios_url     = "https://service.icinga.openology.net/pnp4nagios";
...
my $server_port        = "service.icinga.openology.net:443"; # $server_port        = "nagios.frank4dd.com:80";
my $auth_name          = "Icinga Access"; # $auth_name          = "pnp4nagios";
my $web_user           = "icingaadmin"; # $web_user           = "pnp4nget";
my $web_pass           = "GaTFsttaC2K6"; # $web_pass           = "mypass";
...
# ########################################################################
my $o_smtphost         = "prod2.openology.net";
my $domain 	       = "\@openology.net"; # this is only for -g groups
my @listaddress        = ();

# ########################################################################
# This is the logo image file, the path must point to a valid JPG, GIF or
# PNG file, i.e. the icinga logo. Best size is rectangular up to 160x80px.
# example: [nagioshome]/share/images/NagiosEnterprises-whitebg-112x46.png
# ########################################################################
my $logofile = "/usr/local/icinga/share/images/icinga.jpg";
...
# ########################################################################
# Here I define the HTML color values for each Icinga notification type.
# There is one extra called TEST for sending a test e-mail from the cmdline
# outside of Icinga. The color values are used for highlighting the
...
my %language = ('en' => { 'A' => 'Customer',
                          ...
                          'L' => 'Author',
                          'M' => 'Comment',
                          'N' => 'Icinga Monitoring System Notification',
                          'O' => 'Generated by Icinga, the OpenSource monitoring solution' },
                'fr' => { 'A' => 'Utilisateur',
                          ...
                          'L' => 'Auteur',
                          'M' => 'Commentaire',
                          'N' => 'Notification de la surveillance Icinga',
                          'O' => 'Généré par Icinga, le système de surveillance OpenSource' },
                'de' => { 'A' => 'Anwender',
                          'B' => 'Nachrichtentyp',
                          'C' => 'Dienstname',
                          'D' => 'Dienstzustand',
                          ...
                          'L' => 'Author',
                          'M' => 'Kommentar',
                          'N' => 'Icinga Überwachungssytem Meldung',
                          'O' => 'Erstellt mit Icinga, dem OpenSource Überwachungssytem' },
                'jp' => { 'A' => '顧客名',
                          'B' => '通知の種類',
                          'C' => 'サービス名',
                          'D' => 'サービスの状態',
                          ...
                          'M' => 'コメント',
                          'N' => 'Icinga 監視システムの通知',
                          'O' => 'このメッセージはオープンソースの監視システムIcingaで生成されています。' });

####### Global Variables - No changes necessary below this line ##########
# Icinga notification type, i.e. PROBLEM $NOTIFICATIONTYPE$
my $o_notificationtype = $ARGV[0];
# Icinga notification author (if avail.) $NOTIFICATIONAUTHOR$
my $o_notificationauth = $ARGV[1];
# Icinga notification comment (if avail.) $NOTIFICATIONCOMMENT$
my $o_notificationcmt  = $ARGV[2];
# Icinga service description $SERVICEDESC$
my $o_servicedesc      = $ARGV[3];
# Icinga service state $SERVICESTATE$
my $o_servicestate     = $ARGV[4];
# Icinga service group the service belongs to $SERVICEGROUPNAME$
my $o_servicegroup     = $ARGV[5];
# Icinga monitored host name $HOSTNAME$
my $o_hostname         = $ARGV[6];
# Icinga monitored host alias $HOSTALIAS$
my $o_hostalias        = $ARGV[7];
# Icinga host group the host belongs to $HOSTGROUPNAME$
my $o_hostgroup        = $ARGV[8];
# Icinga monitored host IP address $HOSTADDRESS$
my $o_hostaddress      = $ARGV[9];
# Icinga service check output data $SERVICEOUTPUT$
my $o_serviceoutput    = $ARGV[10];
# Icinga date when the event was recorded $LONGDATETIME$
my $o_datetime         = $ARGV[11];
# The recipients defined in $CONTACTEMAIL$
my $o_to_recipients    = $ARGV[12];
# Modified by Robert Becht for using $CONTACTGROUPEMEMBERS$ in icinga.conf
my $recipient_group    = $ARGV[13];
...
# ########################################################################
sub help {
   print "\nIcinga e-mail notification script for service events, version ",$Version,"\n";
   print "This version was developed for inclusion of PNP4Nagios performance graphs.\n";
   print "GPL licence, (c)2012 Frank Migge\n\n";
   print_usage();
   print <<EOT;

This script takes over Icinga e-mail notifications by receiving the Icinga state
...
-r, --to-recipients
   override the Icinga-provided \$CONTACTEMAIL\$ list of to: recipients
...
-u, --addurl
   this adds URL's to the Icinga web GUI for check status, host and hostgroup
...
Extra: Additional debug output can be generated. Within Icinga, select a host
       ...
       containing a list of values for important Icinga and script variables.
...
sub create_message_text {
  ...
  # if author and comment data has been passed from Icinga
...
sub create_message_html {
  ...
  # The Servicegroup URL http://<icinga-web>/cgi-bin/status.cgi?servicegroup=$SERVICEGROUPNAME$&style=overview
  # This URL shows the service group table listing ofthe hosts that have this service
  if (defined($o_addurl)) {
    $html_msg .= "<a href=\"$icinga_cgiurl/status.cgi?servicegroup=" 
  ...
  # The ServiceOutput URL http://<icinga-web>/cgi-bin/extinfo.cgi?type=2&host=$HOSTNAME$&service=$SERVICEDESC$
  # This URL shows the full service details and commands for service management (ack, re-check, disable, etc)
  if (defined($o_addurl)) {
    $html_msg .= "<a href=\"$icinga_cgiurl/extinfo.cgi?type=2&host=" . urlencode($o_hostname) 
  ...
  # The Hostname URL http://<icinga-web>/cgi-bin/status.cgi?host=$HOSTNAME$&style=detail
  # this URL shows the host and all services underneath it
  if (defined($o_addurl)) {
    $html_msg .= "<a href=\"$icinga_cgiurl/status.cgi?host=" . urlencode($o_hostname) 
  ...
  # The Hostgroup URL http://<icinga-web>/cgi-bin/status.cgi?hostgroup=$HOSTGROUPNAME$&style=overview
  # This URL shows the hostgroup table listing for all individual hosts that belong to it
  if (defined($o_addurl)) {
    $html_msg .= "<a href=\"$icinga_cgiurl/status.cgi?hostgroup=" 
  ...
  # If the author and comment data has been passed from icinga
  # and these variables have content, then we add two more columns
  if ( ( defined($o_notificationauth) && defined($o_notificationcmt) ) &&
       ( ($o_notificationauth ne "") && ($o_notificationcmt ne "") ) ) {
  ...
  # add the Icinga footer tag line here
  $html_msg .= "<br><hr>\n$language{$land}{'O'}\n<hr>\n";
...
sub set_subject {
  ...
  my %lang =  ('en' => "Icinga: $o_notificationtype service $o_servicedesc on $o_hostname ($o_hostgroup) is $o_servicestate",
               'de' => "Icinga: $o_notificationtype $o_hostname($o_hostgroup) mit Dienst $o_servicedesc ist $o_servicestate",
               'jp' => "Icinga: $o_notificationtype $o_hostname($o_hostgroup)".$jp_b64."$o_servicedesc $o_servicestate",
	       'fr' => "Icinga: $o_notificationtype : le service $o_servicedesc sur $o_hostname ($o_hostgroup) est $o_servicestate" );
...
# #######################################################################
# Create a debugging table to check on Icinga and script variables
...
sub create_debugtable() {
  ...
  my %param_vars = (
		'script'  => {	"title"			=>  'Script debug data',
				...
				"boundary"		=>  \$boundary },
		'icinga'  => {	"title"		   	=>  'Icinga debug data',
