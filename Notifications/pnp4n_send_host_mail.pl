...
# The Icinga CGI URL for integrated links
...
my $auth_name          = "Icinga Access"; # $auth_name          = "nagios";
my $web_user           = "icingaadmin"; # $web_user           = "guest";
...
my $domain             = "\@openology.net"; # only for -g group
my @listaddress        = ();
...
my %language = ('en' => { 'A' => 'Customer',
                          ...
                          'J' => 'Author',
                          'K' => 'Comment',
                          'L' => 'Icinga Monitoring System Notification',
                          'M' => 'Generated by Icinga, the OpenSource monitoring solution' },
                'fr' => { 'A' => 'Utilisateur',
                          ...
                          'J' => 'Auteur',
                          'K' => 'Commentaire',
                          'L' => 'Notification de la surveillance Icinga',
                          'M' => 'Généré par Icinga, le système de surveillance OpenSource' },
                'de' => { 'A' => 'Anwender',
                          'B' => 'Nachrichtentyp',
                          'C' => 'Systemzustand',
                          'D' => 'Systemname',
                          ...
                          'J' => 'Author',
                          'K' => 'Kommentar',
                          'L' => 'Icinga Überwachungssytem Meldung',
                          'M' => 'Erstellt mit Icinga, dem OpenSource Überwachungssytem' },
                'jp' => { 'A' => '顧客名',
                          'B' => '通知の種類',
                          'C' => 'ホストの状態',
                          'D' => 'ホスト名',
                          ...
                          'K' => 'コメント',
                          'L' => 'Icinga 監視システムの通知',
                          'M' => 'このメッセージはオープンソースの監視システムIcingaで生成されています。' });
...
my $o_hostname         = $ARGV[3];
# Icinga monitored host alias $HOSTALIAS$
my $o_hostalias        = $ARGV[4];
# Icinga host group the host belongs to $HOSTGROUPNAME$
my $o_hostgroup        = $ARGV[5];
# Icinga monitored host IP address $HOSTADDRESS$
my $o_hostaddress      = $ARGV[6];
# Icinga monitored host state, i.e. DOWN $HOSTSTATE$
my $o_hoststate        = $ARGV[7];
# Icinga monitored host check output data $HOSTOUTPUT$
my $o_hostoutput       = $ARGV[8];
# Icinga date when the event was recorded $LONGDATETIME$
my $o_datetime         = $ARGV[9];
# The recipients defined in $CONTACTEMAIL$
my $o_to_recipients    = $ARGV[10];
# Modified by Robert Becht for using $CONTACTGROUPEMEMBERS$ in icinga.conf
my $recipient_group    = $ARGV[11];
...
sub create_message_html {
  ...
  if (defined($o_addurl)) {
    $html_msg .= "<a href=\"$icinga_cgiurl/status.cgi?host=" . urlencode($o_hostname) ."&style=detail\">$o_hostname</a>";
  ...
  if (defined($o_addurl)) {
    $html_msg = $html_msg
              . "<a href=\"$icinga_cgiurl/status.cgi?hostgroup=" . urlencode($o_hostgroup) ."&style=overview\">$o_hostgroup</a>";
  ...
  if (defined($o_addurl)) {
    ...
    $html_msg .=  "<a href=\"$icinga_cgiurl/status.cgi?type=1&host=" . urlencode($o_hostname) . "\">$o_hostoutput</a>\n";
...
sub set_subject {
  ...
  my %lang =  ('en' => "Icinga: $o_notificationtype Host $o_hostname ($o_hostgroup) is $o_hoststate",
               'de' => "Icinga: $o_notificationtype System $o_hostname($o_hostgroup) ist $o_hoststate",
               'jp' => "Icinga: $o_notificationtype" . $b64_sub . "$o_hoststate",
               'fr' => "Icinga: $o_notificationtype" . $b64_sub . "$o_hoststate" );
